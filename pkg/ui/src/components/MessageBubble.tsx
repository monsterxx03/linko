...
function StreamingToolCallPanel({
  calls,
  isStreaming,
}: {
...
export function MessageBubble({
  ...
  return (
    <div className="flex gap-3">
      {/* Avatar */}
      <div
        className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${roleColors[role].replace("border", "bg")}`}
      >
        {roleIcon[role]}
      </div>
      ...
      <div className="flex-1 max-w-[80%]">
        <div className="flex items-center gap-2 mb-1">
          <span
            className={`text-xs font-medium ${isUser ? "text-indigo-700" : "text-slate-600"}`}
          >
            {roleLabel[role]}
          </span>
          {id && (
